...
def mark(pos, label=None, show_pos=False,
    ...
    pd()
    dot(size, color)
    new_label = label if label is not None else ""
    if show_pos:
        new_label += " " + str(pos)
    if label is not None or show_pos:
        pu()
        goto(add_v(pos, offset))
        pd()
        pen(pencolor=color)
        write(new_label, move=move, align=align, font=font)
    return
    
    
def line(start, end, line_width=1, color="black", mark_vertex=False, vertex_labels=[], 
         show_pos=False, size=5,
         label_color="red", offset=(0.5, 0.5), align="left", font=("Arial", 10, "normal"), move=False):
    pu()
    old_speed = speed()
    goto(start)
    ...
    if mark_vertex:
        for i, point in enumerate(points):
            label = vertex_labels[i] if len(vertex_labels) >= i+1 else ""
            mark(points[i], label, show_pos=show_pos, size=size, 
                 color=label_color, offset=offset, align=align, 
                 font=font, move=move) 
    speed(old_speed)
    
    
def lines(points, line_width=1, color="black", colors=None, fillcolor=None, closed=False, mark_vertex=False, 
          vertex_labels=[], show_pos=False, size=5, label_color="red", offset=(0.5, 0.5), align="left",
          font=("Arial", 10, "normal"), move=False):
    if len(points) < 2:
        return
    pu()
    ...
    if mark_vertex:
        for i,point in enumerate(points):
            label = vertex_labels[i] if len(vertex_labels) >= i+1 else ""
            mark(point, label, show_pos=show_pos, size=size, color=label_color, 
...
def prepare_paper(width, height, scale=20, min_x = 0, min_y = 0, max_x=None, max_y=None, n_tracer=20):
    reset()
